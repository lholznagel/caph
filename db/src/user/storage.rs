...
const METRIC_STORAGE_LOAD: &'static str = "storage::user::load";
const METRIC_STORAGE_SAVE: &'static str = "storage::user::save";

#[async_trait]
impl Storage for UserCache {
    fn file() -> &'static str {
        "./db/storage/users.cachem"
    }
    ...
    async fn load<B>(&self, buf: &mut B) -> Result<(), CachemError>
        ...
        if let Ok(entries) = SaveUsers::read(buf).await {
            let mut map = HashMap::with_capacity(entries.0.len());
            for entry in entries.0 {
                map.insert(entry.user_id, entry);
            }
...
#[derive(Debug, Parse)]
pub struct SaveUsers(pub Vec<UserEntry>);
