...
pub async fn search(req: Request<State>) -> Result<Body> {
    ...
    let result = req
        .state()
        .item_service
        .search(query_params.exact, &query_params.name)
        .await;
    Ok(Body::from_json(&result).unwrap())
}
